...
export default function Auth() {
    ...
    const [hasInvite, setHasInvite] = useState(false); // Toggle for invite code
    const { login, registerHousehold, joinHousehold, joinWithInvite } = useAuth();
        ...
        <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-md p-8 rounded-2xl shadow-xl border border-stone-100">
                <div className="flex flex-col items-center mb-8">
                    <div className="mb-4">
                        <img src="/logo.png" alt="Hearth" className="w-16 h-16 object-contain" />
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <>
                            {isJoin && (
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    />
                                    <label htmlFor="hasInvite" className="text-sm text-stone-600">I have an invite code</label>
                                </div>
                            )}

                            {isJoin && hasInvite ? (
                                <div>
                                    <label className="block text-xs font-medium text-stone-700 mb-1">Invite Code</label>
                                    <input name="inviteCode" type="text" required className="w-full px-3 py-2 bg-stone-50 border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-400 font-mono uppercase tracking-widest" onChange={handleChange} placeholder="XXXXXX" />
                                </div>
                            ) : (
                                <>
                                    <div>
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-stone-900 text-white py-2.5 rounded-xl font-medium hover:bg-stone-800 transition-colors flex justify-center items-center gap-2">
                        ...
                        {isLogin ? 'Sign In' : (isJoin ? (hasInvite ? 'Join with Invite' : 'Join Household') : 'Create Household')}
                    </button>
                </form>
                ...
                <div className="mt-6 flex flex-col items-center gap-2 text-sm text-stone-500">
                    {isLogin ? (
                        <>
                            <button onClick={() => { setIsLogin(false); setIsJoin(false); setHasInvite(false); }} className="hover:text-stone-800">New Household?</button>
                            <button onClick={() => { setIsLogin(false); setIsJoin(true); setHasInvite(false); }} className="hover:text-stone-800">Join Existing Household?</button>
                        </>
                    ) : (
                        <button onClick={() => setIsLogin(true)} className="hover:text-stone-800">Back to Login</button>
                    )}
                    {!isLogin && !isJoin && (
                        <button onClick={() => { setIsJoin(true); setHasInvite(false); }} className="hover:text-stone-800">Join Existing Household instead?</button>
                    )}
                    {!isLogin && isJoin && (
